...
void CDebugInfo::Draw()
{
	...
	if (m_uiDrawDebug || m_uiDrawFPS)
	{
		...
		snprintf(szStr, sizeof(szStr), "%.1f", *pFPS);

		// Dùng toạ độ giống RenderVersion
		pos = ImVec2(pGUI->ScaleX(10.0f), pGUI->ScaleY(10.0f));

		pGUI->RenderText(pos, (ImU32)0xFFFFFFFF, true, szStr);
	}
	...
	snprintf(szStr, sizeof(szStr), "%d/%d %f %d %d",
		msUsed, msAvailable,
		(float)msUsed / (float)msAvailable,
		uiStreamedPeds, uiStreamedVehicles);
