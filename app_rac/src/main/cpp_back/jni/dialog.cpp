	...
	while (std::getline(ssLine, tmpLine, '\n'))
	{
		...
		if (dStyle != DIALOG_STYLE_LIST)
		{
			...
			while (std::getline(ssTabLine, tmpTabLine, '\t'))
			{
				...
				std::string textToDisplay = tmpTabLine;
				if (tmpTabLine.length() >= 8 && tmpTabLine.at(0) == '{' && tmpTabLine.at(7) == '}') {
					lastColor = tmpTabLine.substr(0, 8);
					textToDisplay = tmpTabLine.substr(8);
				}
				
				ImVec2 tabSize;
				tabSize = CalcTextSizeWithoutTags((char*)textToDisplay.c_str());
	...
	for (uint32_t line = dStyle == DIALOG_STYLE_TABLIST_HEADERS ? 1 : 0; line < vLines.size(); line++)
	{
		...
		if (dStyle != DIALOG_STYLE_LIST)
		{
			...
			while (std::getline(ssTabLine, tmpTabLine, '\t'))
			{
				...
				textToDisplay.insert(0, lastColor);
				TextWithColors((char*)textToDisplay.c_str());
		...
		else
		{
            ...
            std::string textToDisplay = vLines[line];
            if (!(vLines[line].length() >= 8 && vLines[line].at(0) == '{' && vLines[line].at(7) == '}')) {
                textToDisplay.insert(0, "{ffffff}");
            }
